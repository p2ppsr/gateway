...
function setDescriptionsIfPayment(
  ...
  if (type === 'payment' && setSpendingDescription_fixed != null && setSpendingDescription_variable != null) {
...
export const initializeIds = async (
  ...
  if (type === 'payment' && description) {
    ...
    effectiveDescription = description.replace(paymentPlaceholder, `paymentId: ${currentId}`)
  }

  // Fallback if no description supplied
  if (!effectiveDescription) {
    effectiveDescription =
      type === 'payment' ? `Payment using paymentId: ${currentId}` : `Payment using buttonId: ${currentId}`
  }
