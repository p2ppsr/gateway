...
const PaymentsList: React.FC = () => {
  ...
  const fetchPayments = async (retries = 2): Promise<void> => {
    ...
    logWithTimestamp(
      F,
      "Starting fetchPayments with statusFilter:",
      statusFilter,
      "Retries:",
      retries,
    );
    try {
      ...
      if (retries > 0) {
        logWithTimestamp(
          F,
          "Retrying fetchPayments, retries left:",
          retries - 1,
        );
        await new Promise((resolve) => setTimeout(resolve, 1000));
        return await fetchPayments(retries - 1);
      }
      setError(message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    logWithTimestamp(
      F,
      "Payments state for target payments:",
  ...
  const paginatedPayments = useMemo(() => {
    ...
    logWithTimestamp(
      F,
      "Paginated payments for target payments:",
      JSON.stringify(targetPaginated, null, 2),
    );
    return paginated;
  ...
  const handleRowsPerPageChange = (
    ...
    if (
      ...
      const numValue =
        typeof value === "number" ? value : (value as any)?.value || 10;
  ...
  const acknowledgePayment = async (paymentId: string | null) => {
    ...
    try {
      ...
      const realIndex = tx.outputs.findIndex((o, idx) => {
        ...
        const lockingScriptHex = String(o.lockingScript.toHex())
          .toLowerCase()
          .trim();
        logWithTimestamp(
          F,
          `🔍 Output[${idx}] lockingScript`,
          lockingScript,
          ",expectedLockingScriptHex=",
          expectedLockingScriptHex,
        );
      ...
      if (realIndex === -1) {
        throw new Error(
          `❌ No output matches expected BRC29 lockingScript for ${paymentId}`,
        );
      }
      ...
      if (atomicBeefTx !== beefRes) {
        console.error(
          `WOC mismatch with DB: atomicBeefTx=${atomicBeefTx}, beefRes=${beefRes}`,
        );
      }
      ...
      {paginatedPayments.length === 0 ? (
        ...
        >
          ...
          >
            ...
            <Typography color="text.secondary">
              It looks like you haven’t received any payments. Check your
              buttons or create a new one!
            </Typography>
            <Button
              variant="contained"
              component={Link}
        ...
        <>
            ...
            <Table>
              ...
              <TableBody>
                {paginatedPayments.map((payment, index) => {
                  ...
                  return (
                    <TableRow
                      key={`${fullPaymentId}-${statusFilter}-${Date.now()}`}
                    >
                      <TableCell>
                      ...
                      <TableCell
                        ref={(el: HTMLTableCellElement | null) =>
                          (columnRefs.current.Txid = el)
                        }
                        onMouseEnter={() =>
                          handleMouseEnter(fullTxid, "Txid", index + 1)
                        }
                      ...
                      <TableCell
                        ref={(el: HTMLTableCellElement | null) =>
                          (columnRefs.current["Payment Id"] = el)
                        }
                        onMouseEnter={() =>
                          handleMouseEnter(
                            fullPaymentId,
                      ...
                      <TableCell
                        ref={(el: HTMLTableCellElement | null) =>
                          (columnRefs.current["Button Id"] = el)
                        }
                        onMouseEnter={() =>
                          handleMouseEnter(fullButtonId, "Button Id", index + 1)
                        }
                        onMouseLeave={handleMouseLeave}
                      ...
                      <TableCell
                        ref={(el: HTMLTableCellElement | null) =>
                          (columnRefs.current["Payer Id"] = el)
                        }
                        onMouseEnter={() =>
                          handleMouseEnter(fullPayerId, "Payer Id", index + 1)
                        }
                        onMouseLeave={handleMouseLeave}
                      ...
                      <TableCell>
                        {payment.amount !== null ? payment.amount : "N/A"}
                      </TableCell>
                      <TableCell>{payment.description || "N/A"}</TableCell>
                      <TableCell
                        data-debug={`completed-${fullPaymentId}-${renderedComplete}`}
                      >
                        {renderedComplete}
                      </TableCell>
                      <TableCell>
          ...
          >
            ...
            <IconButton
              onClick={() =>
                setPage(Math.ceil(payments.length / rowsPerPage) - 1)
              }
              disabled={page >= Math.ceil(payments.length / rowsPerPage) - 1}
              color="inherit"
              size="small"
          ...
          {(hoveredValue || clickedValue) && tableRef.current != null && (
            <Box
              sx={{
                position: "fixed",
            ...
            >
              ...
              >
                ...
                >
                  {clickedValue &&
                  ["Txid", "Payment Id", "Button Id", "Payer Id"].includes(
                    lastClickedColumn || "",
                  ) ? (
