...
const PaymentsList: React.FC = () => {
  ...
  const sortedPayments = useMemo(() => {
    ...
    const sorted = [...payments].sort((a, b) => {
      if (!sortConfig.key || sortConfig.key === 'completed' || sortConfig.key === 'is_new') {
        return 0
      }
  ...
  const handleCustomInputComplete = (
    ...
    if (event.type === 'keypress' && (event as React.KeyboardEvent<HTMLDivElement>).key !== 'Enter') {
      return
    }
  ...
  const MAGIC_BALANCE_BASKET = '893b7646de0e1c9f741bd6e9169b76a8847ae34adef7bef1e6a285371206d2e8'
  ...
  const pendingSats = useMemo(() => {
    return payments.filter(p => p.is_new && typeof p.amount === 'number').reduce((sum, p) => sum + (p.amount ?? 0), 0)
  }, [payments])

  const pendingPaymentIds = useMemo(() => {
    return payments.filter(p => p.is_new && (p.amount ?? 0) > 0 && !!p.payment_id).map(p => p.payment_id as string)
  }, [payments])
  ...
  const acknowledgePayment = async (paymentId: string | null) => {
    ...
    try {
      ...
      const { atomicBeefTx, txid, derivationPrefix, derivationSuffix, merchantId, expectedLockingScriptHex } = data
