...
const PaymentsList = () => {
  ...
  const fetchPayments = async (retries = 2): Promise<void> => {
    ...
    try {
      ...
      logWithTimestamp(F, 'Total mapped payments:', deepCopiedPayments.length, 'Completed count:',
        deepCopiedPayments.filter((p: Payment) => p.completed).length,
        'Incomplete count:', deepCopiedPayments.filter((p: Payment) => !p.completed).length);
  ...
  const sortedPayments = useMemo(() => {
    ...
    logWithTimestamp(F, 'Sorted payments:', sorted.map((p: Payment) => ({
      payment_id: p.payment_id,
      completed: p.completed,
      is_new: p.is_new,
    })));
  ...
  const paginatedPayments = useMemo(() => {
    ...
    logWithTimestamp(F, 'Paginated payments length:', paginated.length, 'Total payments length:', payments.length, 'Page:', page, 'Offset:', page * rowsPerPage, 'Status filter:', statusFilter);
    logWithTimestamp(F, 'Paginated payments:', paginated.map((p: Payment) => ({
      payment_id: p.payment_id,
      completed: p.completed,
      is_new: p.is_new,
    })));
          ...
          >
            ...
            <TablePagination
              ...
              onPageChange={(e, newPage) => {
                setPage(newPage);
                logWithTimestamp(F, 'Page changed to:', newPage, 'Rows:', paginatedPayments, 'Payments length:', payments.length);
                setHoveredValue(null);
                setClickedValue(null);
                setIsClicked(false);
                setExitDirection(null);
