...
import { atomDark, oneLight, vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism'
...
function formatHtml(html: string): string {
  try {
    return (window as any).prettier ? (window as any).prettier.format(html, { parser: 'html' }) : html
  } catch {
    return html
  }
}
...
const CodeSnippet: React.FC<CodeSnippetProps> = ({ code, language }) => {
  ...
  return (
    <Box component={Paper} variant="outlined" sx={{ p: 2, backgroundColor: theme.palette.background.paper }}>
      <SyntaxHighlighter
        language={language}
    ...
    const initializeIfNeeded = async () => {
      ...
      try {
        ...
        if (!identity || !identity.publicKey) {
          throw new Error('Invalid identity returned from wallet')
        }
        ...
        if (walletError.message.includes('Session not found for nonce')) {
          ...
          if (walletConnected) {
            try {
              ...
              if (!identity || !identity.publicKey) {
                throw new Error('Invalid identity after retry')
              }
              merchantId = identity.publicKey
              setMerchant(merchantId)
              setHasMetanet(true)
...
<div class="gateway-paybutton gateway-paybutton-variable" data-variable="true">Pay</div>`
  ...
  const handleCopyCode = async (): Promise<void> => {
    ...
    const updatedDescription = sanitizeInput(description || `Payment using paymentId: ${currentPaymentId}`).slice(0, 80)
    ...
    try {
      ...
      toast.success(
        `${paymentType === 'fixed' ? 'Fixed' : 'Variable'} button copied to clipboard (${
  ...
  .gateway-paybutton-variable:hover {
    ...
    color: #ffffff;
                              ...
                              const variableCode = `<style>\n${extractCSS(
                                value ? variableCSS : `${variableBaseCSS}\n</style>`
            ...
            <Grid item xs={12} md={6}>
              <Card>
                ...
                <Box>
                  <CodeSnippet language="html" code={`<script src="${CONFIG.PAY_BASE}/pay.js" defer></script>`} />
                </Box>
              </Card>
            </Grid>
          </Grid>
