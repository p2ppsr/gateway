...
useEffect(() => {
      ...
      try {
        ...
        const processedButtons = mappedButtons.map(button => {
          if (!button.payment_id && button.customCSS) {
            ...
            logWithTimestamp(F, `Extracted payment_id ${paymentId} from customCSS for button ${button.button_id}`)
  ...
  logWithTimestamp(F, 'Pagination count:filteredButtons.length:', filteredButtons.length.toString())
        ...
        <TableRow key={button.payment_id}>
          ...
          <TableCell
            ref={(el: HTMLTableCellElement | null) => {
              setTimeout(() => {
                if (el) {
                  columnRefs.current['HTML Code'] = el;
...
<div style={{ marginTop: '0.5em', display: 'flex', flexDirection: 'row', width: '100%', position: 'relative', alignItems: 'flex-start' }}>
<Box sx={{ flex: '0 0 auto', paddingRight: '10px' }}>
  {(hoveredValue || clickedValue) && tableRef.current && (
    ...
    >
      <Tooltip title={(hoveredValue && exitDirection === 'bottom') || clickedValue ? 'click to reset' : hoveredValue ? 'click table field' : ''}>
        <Typography
