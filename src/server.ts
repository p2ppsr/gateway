...
async function initializeServer(): Promise<void> {
  try {
    ...
    // --- Rate limiting (buckets + shared options)
    const commonLimiterOpts = {
      standardHeaders: true,
      legacyHeaders: false,
      skip: (req: Request) => {
        if (req.method === 'OPTIONS') return true // don’t throttle preflight
    ...
    const walletProxyLimiter = rateLimit({
      ...commonLimiterOpts,
      windowMs: 60_000,
      max: 60
    })

    // Run limiter on WELL_KNOWN **before** auth router (skip OPTIONS)
    app.use(WELL_KNOWN_PATH, (req, res, next) => {
      if (req.method !== 'OPTIONS') return (wellKnownLimiter as any)(req, res, next)
      next()
    })
    ...
    // Apply per-bucket limiters for remaining routes
    app.use('/wallet-proxy', walletProxyLimiter)
    app.use(ROUTING_PREFIX, apiLimiter)
