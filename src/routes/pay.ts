  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      const serverIdentityKey =
        ...
        (req.app.get("config")?.SERVER_IDENTITY_KEY as string) ||
        "";

      let senderIdentityKey: string | null =
        (req as any).auth?.identityKey || null;
      if (!senderIdentityKey || senderIdentityKey === "unknown")
        senderIdentityKey = null;

      const headerServerKey = req.headers["x-bsv-server"];
      if (
        typeof headerServerKey === "string" &&
        headerServerKey === serverIdentityKey
      ) {
      ...
      if (!senderIdentityKey && allowFallback) {
        senderIdentityKey =
          "0282f7effd932d9d2a3774c287eacb9ace3728a753a0339e2f16998153e5d65963";
      }
      ...
      const expectedLockingScript = BRC29.lock(
        fixedPriv.toString(),
      ...
      const txMatches = bsvtx.outputs.some(
        (x: any) =>
          x.lockingScript.toHex() === expectedLockingScript &&
          Math.abs((x.satoshis ?? 0) - expectedAmount) <= 1,
      );
      ...
      logWithTimestamp(
        F,
        `✅ [pay] Payment completed. TXID: ${transaction.txid}`,
      );
