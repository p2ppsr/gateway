...
export default {
  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!paymentRec) {
        res.status(404).json({ status: 'error', message: 'Payment not found or already completed' })
        return
      }
...
if (typeof headerServerKey === 'string' && headerServerKey === serverIdentityKey) {
  ...
  logWithTimestamp(F, '🔑 [pay] Accepted server identity via header', { senderIdentityKey })
}
...
const txEnvelope = typeof transaction === 'string'
...
const expectedLockingScript = BRC29
  .lock(fixedPriv.toString(), paymentRec.merchant_id)
  .toHex()

// 🔹 Compare with transaction outputs
const expectedAmount = amount ?? paymentRec.amount
const txMatches = bsvtx.outputs.some((x: any) =>
