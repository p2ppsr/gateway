  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (
        ...
        logWithTimestamp(
          F,
          "🔍 [invoice] [Step 3A] Reusing existing derivation for paymentId:",
          {
            paymentId,
            derivationPrefix,
            derivationSuffix,
        ...
        logWithTimestamp(
          F,
          "🔍 [invoice] [Step 3B] Generated new derivation for paymentId:",
          {
            paymentId,
            derivationPrefix,
            derivationSuffix,
      ...
      if (!button.multi_use) {
        ...
        if (
          checkExistingPayment &&
          !checkExistingPayment.completed &&
          checkExistingPayment.txid === null
        ) {
          logWithTimestamp(
            F,
      ...
      const lockingScript = BRC29.lock(
        fixedPriv.toString(),
        button.merchant_id,
      );
      logWithTimestamp(
        F,
        "🔍 [invoice] Generated full BRC29 lockingScript=",
        lockingScript,
      );
      ...
      logWithTimestamp(
        F,
        "🔍 [invoice] Prepared outputs array (fixed-key):",
        outputs,
      );
