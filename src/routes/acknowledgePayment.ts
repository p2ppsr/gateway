...
export default {
  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    logWithTimestamp(F, '✅ Merchant authorized to acknowledge', {
      merchantId
    })
    ...
    try {
      ...
      if (!atomicBeefTx && payment.blockchain_transaction) {
        try {
          ...
          logWithTimestamp(
            F,
            '⚠️ Failed to parse blockchain_transaction JSON',
            {
      ...
      try {
        ...
        logWithTimestamp(F, '🔍 Decoded TX from atomicBeefTx', {
      ...
      const expectedLockingScriptHex = BRC29.lock(
        fixedPriv.toString(),
        merchantId
      ).toHex()
      logWithTimestamp(
        F,
        '🔍 Generated full BRC29 expectedLockingScriptHex=',
