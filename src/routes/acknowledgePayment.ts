...
export default {
  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      let atomicBeefTx: string | undefined = req.body.atomicBeefTx || req.body.transaction?.atomicBeefTx
      ...
      await db('payments').where({ payment_id: paymentId }).update({ is_new: 0, updated_at: db.fn.now() })
