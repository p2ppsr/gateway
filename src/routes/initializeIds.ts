...
export default {
  ...
  func: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    if (!isMerchant && !isServer && !allowFallback) {
      logWithTimestamp(F, '❌ [initializeIds] Sender identity does not match merchantId:', {
        senderIdentityKey,
        merchantId,
        serverIdentityKey
      })
    ...
    let initialGenerated =
      paymentId ||
      buttonId ||
      (await generateAndValidateUniqueId(merchantId, paymentId ? 'payment' : 'button', description, paymentId))
    let id = typeof initialGenerated === 'object' && initialGenerated?.id ? initialGenerated.id : initialGenerated
      ...
      await db.transaction(async trx => {
        ...
        while (attempts < 3) {
          ...
          try {
            ...
            const safePaymentId =
              currentId && typeof currentId === 'object' && 'id' in currentId ? currentId.id : currentId

            const existingPayment =
              targetType === 'payment' ? await trx('payments').where({ payment_id: safePaymentId }).first() : null
            ...
            if (errorMessage.includes('ER_LOCK_DEADLOCK') || errorMessage.includes('ER_QUERY_TIMEOUT')) {
              ...
              attempts++
              {
                const generated = await generateAndValidateUniqueId(merchantId, targetType, description, paymentId)
