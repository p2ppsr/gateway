...
export default {
  ...
  func: async (req: Request, res: Response): Promise<void> => {
    ...
    logWithTimestamp(
      F,
      '[buttonCode] [Step 1] Received request for paymentId:',
      paymentId,
      'Type:',
      typeof paymentId
    )
    try {
      logWithTimestamp(
        F,
        '[buttonCode] [Step 2] Checking database connection...'
      )
      await db.raw('SELECT 1')
      logWithTimestamp(
        F,
        '[buttonCode] [Step 2] Database connection successful'
      )
      const query = db('payment_buttons')
        .leftJoin(
          'payments',
          'payment_buttons.payment_id',
          'payments.payment_id'
        )
      ...
      logWithTimestamp(
        F,
        '[buttonCode] [Step 3] Query constructed:',
        query.toString()
      )
      ...
      logWithTimestamp(
        ...
        (button != null) ? JSON.stringify(button) : 'No record found'
      )
      if (button == null) {
        logWithTimestamp(
          F,
          '[buttonCode] [Step 4] No button found for paymentId:',
          paymentId
        )
        res
          .status(404)
          .json({
            status: 'error',
            message: `No button found for paymentId: ${paymentId}`
          })
        return
      }
      ...
      if (!button.html_code || button.html_code.trim() === '') {
        logWithTimestamp(
          F,
          '[buttonCode] [Step 5] Missing or empty html_code for paymentId:',
          paymentId
        )
        res
          .status(400)
          .json({
            status: 'error',
            message: 'html_code is required and cannot be empty'
          })
        return
      }
      logWithTimestamp(
        F,
        '[buttonCode] [Step 5] Found and validated button:',
        JSON.stringify(button)
      )
      let modifiedCode = button.html_code
      const divId = button.button_id
      const divMatch = modifiedCode.match(
        new RegExp(`<div[^>]*id="${divId}"[^>]*>([\\s\\S]*?)</div>`, 'i')
      )
      ...
      if (div != null) {
        logWithTimestamp(
          F,
          '[buttonCode] [Step 6a] Original div block:',
          div.outerHTML
        )
