...
root.render(<div style={{ padding: 16, fontFamily: 'system-ui' }}>Loading…</div>)

// No top-level await: use a promise chain
loadClientConfig()
  .then((cfg) =>
    findWalletOrigin(cfg.walletLocalPorts).then((walletOrigin) => {
      if (!walletOrigin) {
        console.warn('Wallet not detected on any configured port:', cfg.walletLocalPorts)
      }
