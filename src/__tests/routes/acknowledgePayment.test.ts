...
describe('routes/acknowledgePayment.ts', () => {
  ...
  test('rejects missing paymentId', async () => {
    ...
    expect(logWithTimestamp).toHaveBeenCalledWith('routes/acknowledgePayment', '❌ Missing paymentId')
  ...
  test('successfully acknowledges payment', async () => {
    ...
    expect(logWithTimestamp).toHaveBeenCalledWith('routes/acknowledgePayment', 'Received request body:', {
    ...
    expect(logWithTimestamp).toHaveBeenCalledWith('routes/acknowledgePayment', 'is_new column info:', {
      column_name: 'is_new',
      data_type: 'boolean'
    })
    expect(mockDb.transaction).toHaveBeenCalled()
    expect(paymentsBuilder.update).toHaveBeenCalledWith({ is_new: false })
    expect(logWithTimestamp).toHaveBeenCalledWith('routes/acknowledgePayment', 'Update result:', 1)
    expect(logWithTimestamp).toHaveBeenCalledWith(
      'routes/acknowledgePayment',
  ...
  test('handles no rows updated', async () => {
    ...
    expect(mockDb.transaction).toHaveBeenCalled()
    expect(paymentsBuilder.update).toHaveBeenCalledWith({ is_new: false })
    expect(logWithTimestamp).toHaveBeenCalledWith('routes/acknowledgePayment', 'Update result:', 0)
    expect(logWithTimestamp).toHaveBeenCalledWith(
      'routes/acknowledgePayment',
