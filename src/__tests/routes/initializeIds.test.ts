...
describe('routes/initializeIds.ts', () => {
  ...
  test('rejects invalid merchantId format', async () => {
    ...
    mockReq = {
      body: { merchantId: 'invalidMerchantId', description: 'Test description' },
      headers: {},
  ...
  test('rejects validation errors', async () => {
    (validationResult as unknown as jest.Mock).mockReturnValue({
      ...
      array: jest.fn().mockReturnValue([{ msg: 'Invalid field', type: 'field' }]),
    });
    mockReq = {
      body: { merchantId: 'validMerchantId', description: 'Test description' },
      headers: {},
  ...
  test('successfully inserts new button ID', async () => {
    mockReq = {
      body: { merchantId: 'validMerchantId', description: 'Test description', buttonId: 'validButtonId' },
      headers: {},
      auth: { identityKey: 'validMerchantId' },
    };
    ...
    expect(ensureMerchantExists).toHaveBeenCalledWith(mockDb, 'validMerchantId');
    expect(logWithTimestamp).toHaveBeenCalledWith(
      'routes/initializeIds',
    ...
    expect(jsonSpy).toHaveBeenCalledWith({ status: 'success', id: 'validButtonId' });
  });
  ...
  test('successfully inserts new payment ID with existing button ID', async () => {
    ...
    idsBuilder.first
      .mockResolvedValueOnce({ id: 'validButtonId', type: 'button', merchant_id: 'validMerchantId' }) // Button exists
    ...
    expect(statusSpy).toHaveBeenCalledWith(200);
    expect(jsonSpy).toHaveBeenCalledWith({ status: 'success', id: 'validPaymentId' });
  });
  ...
  test('retries and fails after 3 duplicate ID attempts', async () => {
    mockReq = {
      body: { merchantId: 'validMerchantId', description: 'Test description', buttonId: 'duplicateId' },
      headers: {},
      auth: { identityKey: 'validMerchantId' },
    };
    ...
    const { trx, idsBuilder } = makeTrxMock();
    idsBuilder.first
      .mockResolvedValueOnce({ id: 'duplicateId', type: 'button', merchant_id: 'validMerchantId' })
      .mockResolvedValueOnce({ id: 'duplicateId1', type: 'button', merchant_id: 'validMerchantId' })
      .mockResolvedValueOnce({ id: 'duplicateId2', type: 'button', merchant_id: 'validMerchantId' });
  ...
  test('handles retryable database errors (deadlock/timeout)', async () => {
    ...
    idsBuilder.insert
      .mockImplementationOnce(() => { throw new Error('ER_LOCK_DEADLOCK'); })
