...
describe('utils/merchant.ts', () => {
  ...
  test('inserts new merchant with valid merchantId', async () => {
    ...
    expect(logWithTimestamp).toHaveBeenCalledWith('utils/merchant', '✅ Merchant ensured:', { merchantId })
  })
  ...
  test('rejects with database error', async () => {
    ...
    mockDb('merchants').insert.mockImplementation(async () => await Promise.reject(error))
