...
describe('utils/idGenerator.ts', () => {
  ...
  beforeEach(() => {
    ...
    jest.mocked(mockKnex.transaction).mockImplementation(async cb => await cb(mockTrx))
  })
  ...
  describe('generateAndValidateUniqueId', () => {
    ...
    test('handles duplicate ID in ids table with retry', async () => {
      ;(generateBase58 as jest.Mock).mockReturnValueOnce('123456789ABC').mockReturnValueOnce('987654321XYZ')
      mockTrx.first
