...
describe('utils/idGenerator.ts', () => {
  ...
  beforeEach(() => {
    ...
    (ensureMerchantExists as jest.Mock).mockReset().mockResolvedValue(undefined);
    ...
    jest.mocked(mockKnex.transaction).mockImplementation(async (cb) => cb(mockTrx));
  });
  ...
  describe('generateAndValidateUniqueId', () => {
    test('generates and inserts unique ID successfully', async () => {
      ...
      const result = await generateAndValidateUniqueId(merchantId, 'payment', description);
      expect(result).toEqual({ id: '123456789ABC', description });
      expect(ensureMerchantExists).toHaveBeenCalledWith(expect.anything(), merchantId); // db instance may differ
      ...
      expect(logWithTimestamp).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ id: '123456789ABC', type: 'payment', merchantId, finalDescription: description })
      );
      expect(mockTrx.where).toHaveBeenCalledTimes(2);
      expect(mockTrx.where).toHaveBeenCalledWith({ id: '123456789ABC', type: 'payment', merchant_id: merchantId });
      expect(mockTrx.where).toHaveBeenCalledWith({ payment_id: '123456789ABC' });
    });
    ...
    test('handles duplicate ID in ids table with retry', async () => {
      ...
      expect(logWithTimestamp).toHaveBeenCalledWith(
        expect.any(String),
        '✅ Unique ID generated and inserted:',
        expect.objectContaining({ id: '987654321XYZ', type: 'payment', merchantId, finalDescription: description })
      );
      expect(mockTrx.where).toHaveBeenCalledTimes(4);
      expect(mockTrx.where).toHaveBeenCalledWith({ id: '123456789ABC', type: 'payment', merchant_id: merchantId });
      expect(mockTrx.where).toHaveBeenCalledWith({ id: '987654321XYZ', type: 'payment', merchant_id: merchantId });
    });
    ...
    test('handles duplicate ID in payments table with description update', async () => {
      ...
      mockTrx.first
        ...
        .mockResolvedValueOnce(undefined) // new ID in ids
        .mockResolvedValueOnce(undefined); // new ID in payments
      const result = await generateAndValidateUniqueId(merchantId, 'payment', 'Payment with ID 123456789ABC', '123456789ABC');
      expect(result).toEqual({ id: '987654321XYZ', description: 'Payment with ID 987654321XYZ' });
      expect(mockTrx.update).toHaveBeenCalledWith({ description: 'Payment with ID 987654321XYZ' });
      expect(mockTrx.insert).toHaveBeenCalledWith({
        id: '987654321XYZ',
        merchant_id: merchantId,
        type: 'payment',
      ...
      expect(logWithTimestamp).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ payment_id: '123456789ABC', finalDescription: 'Payment with ID 987654321XYZ' })
      );
      expect(mockTrx.where).toHaveBeenCalledTimes(4);
      expect(mockTrx.where).toHaveBeenCalledWith({ payment_id: '123456789ABC' });
      expect(mockTrx.where).toHaveBeenCalledWith({ id: '987654321XYZ', type: 'payment', merchant_id: merchantId });
    ...
    test('throws error after max attempts for persistent duplicates', async () => {
      ...
      mockTrx.first
        ...
        .mockResolvedValueOnce({ id: '123456789ABC' }) // duplicate in ids
        .mockResolvedValueOnce(undefined);
      await expect(generateAndValidateUniqueId(merchantId, 'payment', description)).rejects.toThrow(
      ...
      expect(logWithTimestamp).toHaveBeenCalledWith(
        expect.any(String),
        '🔍 Checking uniqueness:',
        expect.objectContaining({ id: '123456789ABC', type: 'payment', merchantId, attempt: expect.any(Number) })
      );
    });

    test('throws error for invalid description (empty)', async () => {
      await expect(generateAndValidateUniqueId(merchantId, 'payment', '')).rejects.toThrow(
    ...
    test('throws error for invalid description (too long)', async () => {
      ...
      await expect(generateAndValidateUniqueId(merchantId, 'payment', longDescription)).rejects.toThrow(
        'Description is required and must not exceed 80 characters'
      );
      expect(ensureMerchantExists).not.toHaveBeenCalled();
      expect(mockTrx.insert).not.toHaveBeenCalled();
    ...
    test('throws error for invalid description (non-string)', async () => {
      await expect(generateAndValidateUniqueId(merchantId, 'payment', null as any)).rejects.toThrow(
        'Description is required and must not exceed 80 characters'
      );
      expect(ensureMerchantExists).not.toHaveBeenCalled();
      expect(mockTrx.insert).not.toHaveBeenCalled();
    ...
    test('replaces previousId in description for payment type', async () => {
      mockTrx.first
        ...
        .mockResolvedValueOnce(undefined); // payments table
      const result = await generateAndValidateUniqueId(merchantId, 'payment', 'Payment with ID 123456789ABC', '123456789ABC');
      expect(result).toEqual({ id: '123456789ABC', description: 'Payment with ID 123456789ABC' });
      expect(logWithTimestamp).toHaveBeenCalledWith(
        expect.any(String),
        '🔍 Replaced previous payment_id in description:',
        expect.objectContaining({ oldId: '123456789ABC', newId: '123456789ABC', finalDescription: 'Payment with ID 123456789ABC' })
      );
      expect(mockTrx.insert).toHaveBeenCalledWith({
        id: '123456789ABC',
        merchant_id: merchantId,
    ...
    test('does not replace previousId for button type', async () => {
      mockTrx.first
        .mockResolvedValueOnce(undefined) // ids table
        .mockResolvedValueOnce(undefined); // payments table
      const result = await generateAndValidateUniqueId(merchantId, 'button', 'Payment with ID 123456789ABC', '123456789ABC');
      expect(result).toEqual({ id: '123456789ABC', description: 'Payment with ID 123456789ABC' });
    ...
    test('handles transaction rollback on error', async () => {
      ...
      await expect(generateAndValidateUniqueId(merchantId, 'payment', description)).rejects.toThrow('Database error');
      expect(mockTrx.insert).toHaveBeenCalledWith({
        id: '123456789ABC',
        merchant_id: merchantId,
        type: 'payment',
      ...
      expect(logWithTimestamp).toHaveBeenCalledWith(
        expect.any(String),
        '🔍 Checking uniqueness:',
        expect.objectContaining({ id: '123456789ABC', type: 'payment', merchantId, attempt: 1 })
      );
    });
  });
});
