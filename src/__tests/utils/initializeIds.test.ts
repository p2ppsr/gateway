...
import { fetchWithTimeout, generateBase58, isBase58 } from '../../utils/general';
...
describe('utils/initializeIds.ts', () => {
  ...
  let mockSetSpendingDescriptionFixed: jest.MockedFunction<Dispatch<SetStateAction<string>>>;
  let mockSetSpendingDescriptionVariable: jest.MockedFunction<Dispatch<SetStateAction<string>>>;
  let mockLocalStorage: { getItem: jest.Mock; setItem: jest.Mock; removeItem: jest.Mock };
  ...
  beforeEach(() => {
    ...
    // Mock isBase58
    (isBase58 as jest.Mock).mockReset().mockImplementation((id: string, length: number) => {
      const regex = /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{12}$/;
  ...
  describe('initializeIds', () => {
    test('returns cached ID when initialized and not forced', async () => {
      ...
      expect(mockSetSpendingDescriptionFixed).toHaveBeenCalledWith('Payment using paymentId: 123456789ABC');
      expect(mockSetSpendingDescriptionVariable).toHaveBeenCalledWith('Payment using paymentId: 123456789ABC');
      expect(logWithTimestamp).toHaveBeenCalledWith(
        expect.any(String),
        expect.stringContaining('initializeIds: paymentId already initialized for merchant aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa, using cached ID: 123456789ABC')
      );
...
test('generates new ID for invalid cache state', async () => {
  ...
  (fetchWithTimeout as jest.Mock).mockReset().mockResolvedValue({
    ok: true,
    status: 200,
    text: jest.fn().mockResolvedValue('{"status":"success","id":"987654321XYZ"}'),
  });
  const result = await initializeIds('payment', mockWallet, undefined, 'a'.repeat(64), mockSetId, mockSetSpendingDescriptionFixed, mockSetSpendingDescriptionVariable);
  expect(result).toEqual({ status: 'success', id: '987654321XYZ' });
  expect(mockSetId).toHaveBeenCalledWith('987654321XYZ');
  ...
  expect(fetchWithTimeout).toHaveBeenCalledWith(
    ...
    test('validates and stores new payment ID successfully', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId, mockSetSpendingDescriptionFixed, mockSetSpendingDescriptionVariable);
      expect(result).toEqual({ status: 'success', id: '123456789ABC' });
      expect(mockSetId).toHaveBeenCalledWith('123456789ABC');
      expect(mockSetSpendingDescriptionFixed).toHaveBeenCalledWith('Payment using paymentId: 123456789ABC');
      expect(mockSetSpendingDescriptionVariable).toHaveBeenCalledWith('Payment using paymentId: 123456789ABC');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('idsInitializedpayment_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'true');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('paymentId_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '123456789ABC');
      expect(fetchWithTimeout).toHaveBeenCalledWith(
        'http://localhost:3000/api/initializeIds',
        expect.objectContaining({
          method: 'POST',
    ...
    test('validates and stores new button ID successfully', async () => {
      ...
      const result = await initializeIds('button', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId);
      expect(result).toEqual({ status: 'success', id: '123456789ABC' });
      expect(mockSetId).toHaveBeenCalledWith('123456789ABC');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('idsInitializedbutton_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'true');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('buttonId_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '123456789ABC');
      expect(fetchWithTimeout).toHaveBeenCalledWith(
        'http://localhost:3000/api/initializeIds',
        expect.objectContaining({
          method: 'POST',
    ...
    test('retries on 409 response with new ID', async () => {
      ...
      (fetchWithTimeout as jest.Mock)
        ...
        .mockResolvedValueOnce({
          ok: false,
          status: 409,
          text: jest.fn().mockResolvedValue('{"status":"error","message":"Duplicate ID","newId":"987654321XYZ"}'),
        })
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
    ...
    test('retries on 409 response with invalid newId and generates local ID', async () => {
      ...
      (fetchWithTimeout as jest.Mock)
        ...
        .mockResolvedValueOnce({
          ok: false,
          status: 409,
          text: jest.fn().mockResolvedValue('{"status":"error","message":"Duplicate ID","newId":"invalid"}'),
        })
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
    ...
    test('returns error after max retry attempts', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId);
      expect(result).toEqual({ status: 'error', message: 'Failed to validate paymentId after 3 attempts' });
      expect(fetchWithTimeout).toHaveBeenCalledTimes(3);
      expect(toast.error).toHaveBeenCalledWith('Failed to validate paymentId after 3 attempts');
    });
    ...
    test('returns error for invalid ID length with no cache', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '123', 'a'.repeat(64), mockSetId);
      expect(result).toEqual({ status: 'success', id: '987654321XYZ' });
      expect(generateBase58).toHaveBeenCalledWith(12);
      expect(fetchWithTimeout).toHaveBeenCalledWith(
        'http://localhost:3000/api/initializeIds',
    ...
    test('returns error for wallet unavailable', async () => {
      ...
      mockLocalStorage.getItem
        .mockReturnValueOnce(null) // isInitialized
        .mockReturnValueOnce(null); // storedId
      const result = await initializeIds('payment', mockWallet, '123456789ABC', undefined, mockSetId);
      expect(result).toEqual({ status: 'error', message: 'Failed to get merchant ID: Wallet offline' });
      expect(toast.error).toHaveBeenCalledWith('Failed to get merchant ID: Wallet offline');
      expect(fetchWithTimeout).not.toHaveBeenCalled();
    });
    ...
    test('uses force flag to bypass cache', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '987654321XYZ', 'a'.repeat(64), mockSetId, mockSetSpendingDescriptionFixed, mockSetSpendingDescriptionVariable, undefined, true);
      expect(result).toEqual({ status: 'success', id: '987654321XYZ' });
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('idsInitializedpayment_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'true');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('paymentId_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '987654321XYZ');
    ...
    test('returns error for API failure with 400 response', async () => {
      ...
      (fetchWithTimeout as jest.Mock).mockReset().mockResolvedValue({
        ok: false,
        status: 400,
        text: jest.fn().mockResolvedValue('{"status":"error","message":"Invalid request"}'),
      });
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId);
      expect(result).toEqual({ status: 'error', message: 'Failed to validate paymentId: Invalid request (Status: 400)' });
      expect(toast.error).toHaveBeenCalledWith('Failed to validate paymentId: Invalid request (Status: 400)');
    });
    ...
    test('returns error for fetch error', async () => {
      mockLocalStorage.getItem
        .mockReturnValueOnce(null) // isInitialized
        .mockReturnValueOnce(null); // storedId
      (fetchWithTimeout as jest.Mock).mockReset().mockRejectedValue(new Error('Network error'));
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId);
    ...
    test('uses buttonId for payment type', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId, mockSetSpendingDescriptionFixed, mockSetSpendingDescriptionVariable, '987654321XYZ');
      expect(result).toEqual({ status: 'success', id: '123456789ABC' });
      expect(fetchWithTimeout).toHaveBeenCalledWith(
        'http://localhost:3000/api/initializeIds',
        expect.objectContaining({
    ...
    test('returns error for invalid buttonId', async () => {
      mockLocalStorage.getItem
        .mockReturnValueOnce(null) // isInitialized
        .mockReturnValueOnce(null); // storedId
      (isBase58 as jest.Mock).mockReset().mockImplementation((id: string) => id === '123456789ABC');
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId, mockSetSpendingDescriptionFixed, mockSetSpendingDescriptionVariable, 'invalid');
      expect(result).toEqual({ status: 'error', message: 'Invalid 12-character Base58 buttonId provided' });
      expect(toast.error).toHaveBeenCalledWith('Invalid 12-character Base58 buttonId provided');
      expect(fetchWithTimeout).not.toHaveBeenCalled();
    });
    ...
    test('handles missing setId and setSpendingDescription', async () => {
      ...
      const result = await initializeIds('payment', mockWallet, '123456789ABC', 'a'.repeat(64));
      expect(result).toEqual({ status: 'success', id: '123456789ABC' });
    ...
    test('returns error for invalid type', async () => {
      const result = await initializeIds('invalid' as any, mockWallet, '123456789ABC', 'a'.repeat(64), mockSetId);
      expect(result).toEqual({ status: 'error', message: "Invalid type: invalid. Must be 'payment' or 'button'" });
      expect(toast.error).toHaveBeenCalledWith("Invalid type: invalid. Must be 'payment' or 'button'");
      expect(fetchWithTimeout).not.toHaveBeenCalled();
    });
  });
});
