...
describe('utils/getLatestMetanetclientLinks.ts', () => {
  ...
  test('returns null links for invalid GitHub response', async () => {
    ...
    expect(mockResponse.json).toHaveBeenCalled()
    expect(logWithTimestamp).not.toHaveBeenCalledWith(expect.any(String), '🔍 GitHub release data:', expect.anything())
    expect(logWithTimestamp).not.toHaveBeenCalledWith(
      expect.any(String),
      '✅ Successfully fetched Metanet client links:',
      expect.anything()
    )
    expect(console.error).toHaveBeenCalledWith('❌ Failed to fetch latest Metanet client release:', expect.any(Error))
    expect(result).toEqual(expectedLinks)
  })
  ...
  test('returns null links on network error', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith('❌ Failed to fetch latest Metanet client release:', error)
    expect(result).toEqual(expectedLinks)
  })
})
