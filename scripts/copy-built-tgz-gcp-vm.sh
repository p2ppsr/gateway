...
gcloud compute ssh "$VM" --zone="$ZONE" --command 'sudo mkdir -p /srv/gateway/releases && sudo chown -R bob:bob /srv/gateway'

echo "== copy to /srv/gateway/releases/ =="
gcloud compute scp "$ARTIFACT" "$VM":/srv/gateway/releases/ --zone "$ZONE"
