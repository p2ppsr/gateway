...
cd "$ROOT"
if [[ -f scripts/watch-pay.sh && ! -f scripts/watch-pay.sh.orig.bak ]]; then
  cp scripts/watch-pay.sh scripts/watch-pay.sh.orig.bak
fi
cat > scripts/watch-pay.sh <<'S'
#!/usr/bin/env bash
set -euo pipefail
...
echo "✅ CPR-0003 applied (robust watcher installed)"
