...
declare global {
  interface Window { CONFIG?: { WALLET_ORIGIN?: string } }
}
...
async function tryBind(mode: 'auto' | 'http', origin: string) {
  ...
  try {
    ...
    console.warn('[demo/demoIdsAuth] bind failed:', { mode, origin, error: e?.message ?? String(e) })
    return null
  }
}
...
const App: React.FC = () => {
  ...
  const run = async (type: 'payment' | 'button') => {
    ...
    const res = await initializeIds(type, wallet, undefined, undefined, setLastId)
  ...
  return (
    <div style={{ padding: 16, fontFamily: 'system-ui', lineHeight: 1.45 }}>
      <h3 style={{ marginTop: 0 }}>Initialize IDs (signed via auth middleware)</h3>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button onClick={() => run('button')} disabled={!wallet}>Init Button ID</button>
        <button onClick={() => run('payment')} disabled={!wallet}>Init Payment ID</button>
      </div>
