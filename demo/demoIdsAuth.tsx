  ...
  return (
    <div style={{ padding: 16, fontFamily: 'system-ui', lineHeight: 1.45 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button onClick={connectWallet} disabled={!wallet}>Connect Wallet</button>
        <button onClick={createActionDemo} disabled={!wallet}>Run createAction demo</button>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button onClick={() => runInit('button')} disabled={!wallet}>Init Button ID</button>
        <button onClick={() => runInit('payment')} disabled={!wallet}>Init Payment ID</button>
      </div>
