 ...
 */
const { SQL_DATABASE_HOST, SQL_DATABASE_USER, SQL_DATABASE_PASSWORD, SQL_DATABASE_DB_NAME, SQL_DATABASE_PORT } =
  process.env
...
if (missing.length > 0) {
  throw new Error(`❌ Missing/invalid database environment variables: ${missing.join(', ')}`)
}
