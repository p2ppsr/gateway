...
export async function up(knex: Knex): Promise<void> {
  try {
    ...
    if (hasDescriptionInPayments) {
      ...
      console.log('Replaced NULL and altered "description" column in payments to VARCHAR(80) NOT NULL with default empty string.');
    } else {
      await knex.schema.table('payments', (table) => {
    ...
    if (hasMerchantIdInMerchants) {
        ...
        if (fkExists) {
          try {
            ...
            if (tableInfo) {
              ...
              console.log('Explicitly dropped foreign key constraint payments_merchant_id_foreign on payments.merchant_id.');
            }
          } catch (error) {
            console.warn(`Warning: Could not drop foreign key on ${table}.merchant_id:`, error instanceof Error ? error.message : 'Unknown error');
          }
        }
      }
      ...
      for (const table of tablesWithFK) {
        ...
        if (fkExists) {
          await knex.schema.table(table, (table) => {
            table.foreign('merchant_id').references('merchant_id').inTable('merchants').onDelete('CASCADE').onUpdate('CASCADE');
          });
    ...
    if (hasMerchantIdInIds) {
      ...
      if (idsFKInfo && idsFKInfo.REFERENCED_TABLE_NAME === 'merchants' && idsFKInfo.REFERENCED_COLUMN_NAME === 'merchant_id') {
        await knex.schema.table('ids', (table) => {
          table.foreign('merchant_id').references('merchant_id').inTable('merchants').onDelete('CASCADE').onUpdate('CASCADE');
        });
